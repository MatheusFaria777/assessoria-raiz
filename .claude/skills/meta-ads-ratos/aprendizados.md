...
### 2026-06-01 — CTA para campanha de visita ao perfil Instagram
**Regra:** O CTA correto para campanha de visita ao perfil Instagram é `VIEW_INSTAGRAM_PROFILE` (não `VISIT_INSTAGRAM_PROFILE`). Passar no criativo como `call_to_action: {"type": "VIEW_INSTAGRAM_PROFILE", "value": {"link": "https://www.instagram.com/USUARIO/"}}`.
**Contexto:** API retornou erro 100 com `VISIT_INSTAGRAM_PROFILE`. O tipo aceito na lista da API é `VIEW_INSTAGRAM_PROFILE`.

### 2026-06-01 — Criar campanha de tráfego requer is_adset_budget_sharing_enabled
**Regra:** Ao criar campanhas com `OUTCOME_TRAFFIC` (ABO), passar `is_adset_budget_sharing_enabled: False`. Sem esse parâmetro, a API retorna erro 4834011.
**Contexto:** Criação de campanha de tráfego para MK Beauty falhou sem esse campo.

### 2026-06-01 — Ad set de visita ao perfil Instagram requer bid_strategy explícito
**Regra:** Ao criar ad sets com `optimization_goal: VISIT_INSTAGRAM_PROFILE`, sempre passar `bid_strategy: LOWEST_COST_WITHOUT_CAP`. Sem isso, a API retorna erro 2490487 exigindo valor de lance.
**Contexto:** Criação do Mix frios para MK Beauty falhou sem o bid_strategy.

### 2026-06-01 — Criativos de catálogo dinâmico não podem ser reusados via object_story_id
**Regra:** Posts criados originalmente via Dynamic Ads (catálogo) não podem ser promovidos em novas campanhas via `object_story_id` sem passar o `product_set_id`. Identificar pelo nome do criativo com `{{product.name}}`. Para esses casos, criar o ad manualmente no Ads Manager ou usar outro post.
**Contexto:** AD92 da MK Beauty (creative 1767355417262051) usava post dinâmico. API retornou erro 1815017.
