...
def _resolve_fields(args):
    ...
    Quando --level esta definido, injeta automaticamente os campos de ID/nome
    do nivel correspondente no inicio da lista de campos.
    """
    base = parse_fields(args.fields) if args.fields else list(DEFAULT_INSIGHTS_FIELDS)
    level = getattr(args, "level", None)
    if level and level in _LEVEL_ID_FIELDS:
        prefix = _LEVEL_ID_FIELDS[level]
        # Adiciona apenas os que ainda nao estao na lista
        base = [f for f in prefix if f not in base] + base
    return base
