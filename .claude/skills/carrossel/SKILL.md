...
1. Verificar se existe algum arquivo em `conteudo/planejamento/ideias/`. Se existir ao menos um, listar os títulos (campo `titulo` do frontmatter) e perguntar:
   > "Tem [N] ideia(s) salva(s) aqui:
   > [lista numerada dos títulos]
   > Quer usar alguma delas ou partir do zero?"

   Se o usuário escolher uma ideia, ler o arquivo correspondente e usá-lo como ponto de partida. Se quiser partir do zero, seguir normalmente.

   Se `conteudo/planejamento/ideias/` estiver vazio ou não existir, pular essa etapa sem mencionar.

2. Ler `_contexto/preferencias.md` pra calibrar tom
2. Ler `_contexto/empresa.md` pra entender contexto e público
3. Ler `conteudo/referencias/referencia-mentora.md` pra calibrar estrutura narrativa, padrão de títulos e tom — o conteúdo da Assessoria Raiz segue esse estilo como referência base
4. Se o input for um link, buscar o conteúdo com os fallbacks corretos:
   ...
   - **Links do Instagram (post/reel):** WebFetch não acessa conteúdo autenticado. Tentar primeiro baixar o áudio via yt-dlp e transcrever com faster-whisper (ambos instalados):
     ```bash
     yt-dlp -x --audio-format mp3 -o "dados/temp/%(id)s.%(ext)s" "URL_DO_REEL"
     python -c "from faster_whisper import WhisperModel; m = WhisperModel('small', device='cpu', compute_type='int8'); segs, _ = m.transcribe('dados/temp/ID.mp3', language='pt'); [print(s.text) for s in segs]"
     ```
     Se falhar (reel privado, erro de autenticação), pedir ao usuário pra copiar a legenda/texto do post ou descrever o conteúdo
   - **Links de YouTube:** se tiver a skill `/yt-transcript` ou `/transcribe`, usar. Se não, tentar WebFetch na página
5. **Se o usuário mencionar algo que tu não conhece ou não tem certeza do que é** (nome de produto, ferramenta, evento, pessoa, termo técnico): pesquisar antes de escrever. Usar WebSearch, WebFetch ou qualquer ferramenta disponível. Nunca chutar, corrigir o usuário ou assumir que ele errou. Se ele disse "OpenClaw", vai atrás pra descobrir o que é OpenClaw. O usuário sabe do que tá falando, tu é que pode não saber
6. Definir o ângulo do carrossel. Se vier briefing do /pesquisar-tema com driver emocional e tom definidos, usar como base. Se não, escolher entre: educacional, oportunidade, contrário, provocativo ou inspiracional
...
7. **Planejar a espinha dorsal do carrossel e mostrar pro usuário.** Montar e apresentar:
   ...
   >
   > **Formato narrativo:** [escolher o mais adequado ao driver emocional do ângulo]
   > - **Problema → Solução:** apresenta a dor, explica a causa, entrega a saída
   > - **Listicle:** múltiplos pontos com progressão de peso (o mais pesado por último)
   > - **Editorial:** tese forte + argumentação + virada
   > - **Mito vs Realidade:** crença comum → por que está errada → o que é verdadeiro
   > - **Storytelling:** case real com situação → estratégia → resultado numérico
   >
...
8. **Escrever os slides com base na espinha dorsal e capa aprovadas.** Seguir o arco narrativo:
   ...
   **Slide 3 (Primeira sacada — ponto de virada):** Entrega o primeiro insight real. Mas atenção: o slide 3 não pode resolver a tensão — ele precisa aprofundá-la. Quem passa do slide 3 tende a terminar o carrossel. Quem sai, sai antes. Se o slide 3 fechar o loop ("é isso, entendi"), a pessoa vai embora satisfeita. O objetivo é que a pessoa termine o slide 3 pensando "faz sentido... mas então por quê X?" — não "já entendi, pode parar". Dado concreto aqui se possível: número + fonte + ano.

   **Slide 4 (Mecanismo):** Continua explicando o POR QUE com mais profundidade. Expande o que o slide 3 abriu. Dados ou exemplos específicos (nome de empresa, produto, caso). Nunca genérico.
...
- **Teste do slide 3:** ao terminar de ler o slide 3, a pessoa ficaria curiosa pra saber o próximo ou satisfeita pra sair? Se satisfeita, o slide 3 resolveu cedo demais — reescrever pra aprofundar a tensão em vez de fechá-la
...
11. Salvar tudo em `conteudo/carrosseis/[tema]/carousel-text.md`
...
## Veto conditions — não entregar se qualquer item for verdadeiro

- Slide 1 sem tensão ou gancho — capa descritiva não para o scroll
- Afirmação factual sem dado específico (número + fonte + ano) — se não tiver dado, usar exemplo real, nunca genérico
- CTA genérico no último slide ("siga para mais", "espero que tenha gostado")
- Caption sem hook standalone nos primeiros 125 caracteres
- O conteúdo narra o fato mas não revela o mecanismo por trás
- Slide 3 que resolve a tensão completamente — se fechar o loop cedo, a maioria vai sair antes do meio. O slide 3 precisa ser a primeira sacada que abre mais perguntas, não a conclusão

Se qualquer condição for verdadeira, reescrever antes de entregar ao checkpoint.
...
Após entregar o carrossel finalizado (todos os PNGs gerados e aprovados pelo usuário), perguntar:

> "O carrossel ficou bom? Quer que eu já adicione no pipeline como 'Pronto pra publicar'?
> Quando você postar, é só chamar `/registrar-conteudo` pra registrar a data e mover pra Publicado."

Se sim: abrir `conteudo/omatheusfariaa/pipeline.md` e adicionar na seção "✅ Pronto" com título, formato Carrossel e caminho `conteudo/carrosseis/[tema]/`.
