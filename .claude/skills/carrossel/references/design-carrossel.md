...
## Aprendizados de produção (Bombril, mai/2026)

- **Não usar labels de seção nos slides** (ex: "HOOK", "A HISTÓRIA", "A VIRADA") — parecem metadado interno, não design. Remover por padrão
- **Números grandes: não usar Agatho** — o typeface tem caracteres angulados (especialmente `%`) que fazem o número parecer torto. Para display de números, usar **Bebas Neue** (Google Fonts) ou outra sans-serif display neutra
- **Símbolo `%` separado do número** — colocar em `<span>` separado com `font-family` e alinhamento próprio quando necessário. Container com `display: flex; align-items: flex-start`
- **Split vertical não funciona para foto de produto** — funciona bem para retrato de pessoa (rosto + contexto). Para produto isolado (caixa, embalagem), usar **foto como cabeçalho horizontal**: `height` fixo no topo + `background-size: contain` + `background-repeat: no-repeat` + gradiente `::after` na borda inferior
- **`background-size: cover` corta produto** — para imagens de produto que precisam aparecer inteiras, usar `background-size: contain` ou valor percentual (ex: `75%`)
- **Número sobrepondo dois blocos de cor** parece "jogado ali" — se quiser número grande com fundo dividido, usar o número como elemento decorativo de fundo (opacity 3-8%) e montar o layout completo em cima, não deixar o número como único elemento do bloco
- **`height` do bloco de foto** controla onde fica a divisão entre imagem e texto — deixar claro pro usuário que é esse valor quando quiser mover a "borda"

## Aprendizados de produção (Coca vs Pepsi, mai/2026)

- **Agatho não funciona em Playwright** — versão disponível é "personal use only" e substitui caracteres acentuados (ã, ç, é, etc.) por watermarks "PLEASE BUY FONT". Stack correto pra carrosséis: **Playfair Display** (headlines, Google Fonts) + **Inter** (corpo) + **Bebas Neue** (números grandes). Nunca usar @font-face com Agatho em HTMLs de carrossel
- **Logo transparente > logo com fundo** — `raiz com fundo transparente.png` integra melhor em slides escuros do que `logo-bege-fundo-verde.png` (que deixa caixa verde visível). Usar o transparente por padrão no slide de CTA
- **Imagem no topo + texto abaixo funciona bem** — layout com `height` fixo pra imagem (ex: 530px) + gradiente `::after` na borda inferior + texto na área restante. Funciona especialmente pra imagens de contexto ou ilustrativas
- **Fundir slides com ponto temático igual** — quando dois slides adjacentes falam do mesmo mecanismo, fundir em um. Mais impacto, menos swipe
- **Grep por traços antes de entregar** — checar `—` em todos os HTMLs antes de renderizar. Substituir por vírgula ou ponto
